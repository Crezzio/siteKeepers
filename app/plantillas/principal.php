<?php $datosUsuario = (new usuarioRepositorio())->datosUsuarioPorId($_SESSION['id']);?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<div class="container" style="margin-top: 30px; margin-bottom: 30px; min-height: 600px">
    <div class="row">
                ...
                <div class="card-body">
                    ...
                    <p class="card-text">
                        <?php if($datosUsuario->propiedadSeteada('descripcion')):?>
                            <i>"<?= $datosUsuario->getDescripcion()?>"</i>
                        <?php else: ?>
        ...
        <div class="col-sm-8">
            <div class="rounded mx-1 mt-1 pt-3 pb-1 shadow" style="background-color: #fff">
                <div class="form-group mx-3">
                    <label for="txBusqueda" class="sr-only">Búsqueda</label>
                    <div class="input-group">
                        <input data-toggle="tooltip" title="Filtra los resultados" type="text" id="txBusqueda" name="txBusqueda" class="form-control" placeholder="Búsqueda por nombre..." required autofocus >
            ...
            <div class="media rounded m-1 p-3 shadow" style="background: linear-gradient(45deg, #8e9eab 0%, #eef2f3 100%); border: 1px solid #e9ecef">
                <a href="index.php?ctl=ficha&id=<?= $valor->getId()?>"><img data-toggle="tooltip" <?php if ($valor->propiedadSeteada('imagen')): ?>title="Mira su perfil" src="web/img/<?= $valor->getImagen()?>"<?php else:?>title="No tiene foto de perfil aún" src="web/img/avatarDefault.jpg"<?php endif; ?> alt="avatar-<?= $valor->getNombre()?>" class="mr-3 mt-3 rounded-circle shadow-sm" style="width: 60px"></a>
                <div class="media-body">
                    <h4 class="mediaTitle font-weight-bold"><?php echo $valor->getNombre().' '.$valor->getApellido1().' '.$valor->getApellido2()?></h4>
                    <blockquote class="blockquote mb-1">
                        <p style="font-size: 16px; font-style: italic">
                            <?php if($valor->propiedadSeteada('descripcion')):?>
                                "<?= $valor->getDescripcion()?>"
                            <?php else: ?>
                                El usuario aún no ha introducido una descripción.
                            <?php endif; ?>
                        </p>
                        <footer class="blockquote-footer">
                            <?php if ($valor->getRol() === 'siteKeeper'): ?><span class="badge badge-success">Site Keeper</span>
                            <?php elseif ($valor->getRol() === 'cliente'): ?><span class="badge badge-info">Cliente</span>
                            <?php elseif ($valor->getRol() === 'ambos'): ?><span class="badge badge-success">Site Keeper</span> <span class="badge badge-info">Cliente</span>
                            <?php elseif ($valor->getRol() === 'admin'): ?><span class="badge badge-secondary">Admin</span><?php endif; ?>
                        </footer>
                    </blockquote>
                </div>
            </div>
